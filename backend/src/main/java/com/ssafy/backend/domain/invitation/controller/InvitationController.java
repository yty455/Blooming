...
public class InvitationController {
    ...
    public ResponseEntity<BasicResponse> registInvitation(@RequestBody InvitationRegistDto invitationRegistDto) throws Exception {
        //유저 찾기
        ...
        //커플에 해당하는 청첩장이 이미 있으면 안됨
        Invitation original = invitationRepository.findByCoupleId(couple.getId());
        if (original != null){
            //이미 청첩장이 존재하면
            basicResponse = BasicResponse.builder()
                    .code(HttpStatus.NO_CONTENT.value())
                    .httpStatus(HttpStatus.NO_CONTENT)
                    .message("커플의 청첩장이 이미 존재합니다.").build();
        }
        else{
